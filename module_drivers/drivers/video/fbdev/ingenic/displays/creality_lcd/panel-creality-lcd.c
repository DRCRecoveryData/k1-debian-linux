...
static struct fb_videomode panel_modes[] = {
	[0] = {
		...
		/* 
		 * Пересчет под чуть большие отступы:
		 * H_Total = 480 + 32 + 32 + 10 = 554
		 * V_Total = 800 + 20 + 12 + 10 = 842
		 * Clock = 554 * 842 * 50 = 23,323,400 Hz
		 * Pixclock = 10^12 / 23323400 = 42875
		 */
		.pixclock       = 42875,
		
		/* 
		 * БЫЛО: 24. СТАЛО: 32.
		 * Это изменение убьет "редкие артефакты".
		 */
		.left_margin    = 32,
		.right_margin   = 32,
		.hsync_len      = 10,
		
		/* Эти значения берем из твоего удачного конфига */
		.upper_margin   = 20,
		.lower_margin   = 12,   // Совпадает с 0x0C в init
		.vsync_len      = 10,   // Длинный импульс держит картинку
		
		/* ОСТАВЛЯЕМ КАК БЫЛО (раз это убирает дрожание) */
