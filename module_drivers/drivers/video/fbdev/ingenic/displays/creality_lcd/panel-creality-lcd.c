...
static struct fb_videomode panel_modes[] = {
	[0] = {
		...
		/* 
		 * Пересчет частоты под увеличенные боковые поля (32 вместо 24).
		 * Это сохранит ровно 50Гц.
		 * Pixclock = 42875
		 */
		.pixclock       = 42875,
		...
		/* 
		 * БЫЛО: 24. СТАЛО: 32.
		 * Это единственное изменение, чтобы убрать редкие полосы.
		 */
		.left_margin    = 32,
		.right_margin   = 32,
		
		/* Все остальное — строго из твоего рабочего конфига */
		.hsync_len      = 10,
		
		.upper_margin   = 20,
		.lower_margin   = 12,   // Оставляем 12, раз оно не дрожит
		.vsync_len      = 10,   // Оставляем 10, это держит кадр
		
		/* Оставляем HIGH_ACT, раз на нем нет дрожания */
