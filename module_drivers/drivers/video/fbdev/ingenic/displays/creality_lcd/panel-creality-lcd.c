...
static struct fb_videomode panel_modes[] = {
	[0] = {
		...
		/* 
		 * ~23 MHz. Стабильная частота.
		 */
		.pixclock       = 43478,
		
		/* Оставляем широкие поля, они защищают от артефактов */
		.left_margin    = 30,
		.right_margin   = 30,
		
		/* 
		 * Уменьшаем HSYNC с 20 до 10.
		 * Слишком длинный импульс размывает начало строки -> дрожание.
		 */
		.hsync_len      = 10,
		
		/* Оставляем 20, чтобы совпадало с init-кодом экрана */
		.upper_margin   = 20,
		.lower_margin   = 12,
		
		/* 
		 * Уменьшаем VSYNC с 10 до 4.
		 * Короткий, четкий импульс лучше фиксирует кадр по вертикали.
		 */
		.vsync_len      = 4,
		
		/* 
		 * ГЛАВНОЕ ИЗМЕНЕНИЕ: 
		 * Добавляем 0x40000000 (FB_SYNC_CLK_LAT_FALLING).
		 * Это заставит экран считывать данные "на полутакте", 
		 * когда сигнал максимально стабилен. Это уберет дрожание.
		 */
