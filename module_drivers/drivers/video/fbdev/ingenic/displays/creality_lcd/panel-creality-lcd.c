...
static struct fb_videomode panel_modes[] = {
	[0] = {
		...
		/* Горизонтальные отступы (30/30) - защита от артефактов */
		.left_margin    = 30,
		.right_margin   = 30,
		.hsync_len      = 10,
		
		/* !!! Вертикальная симметрия для стабильности !!! */
		.upper_margin   = 20,   // Совпадает с первым байтом 0xC1
		.lower_margin   = 20,   // Совпадает со вторым байтом 0xC1
		
		/* 
		 * Делаем VSYNC очень коротким (2 линии).
		 * Длинный импульс может "размазывать" начало кадра и вызывать дрожание.
		 */
		.vsync_len      = 2,
