    ...
    for (i = 0; i < DPU_SUPPORT_MAX_LAYERS; i++) {
        if (lay_cfg[i].lay_en) {
            if (lay_cfg[i].tlb_en != (glbc >> i & 0x1)) {
                glbc = lay_cfg[i].tlb_en ? (glbc | (0x1 << i)) : (glbc & ~(0x1 << i));
            }
        } else {
            glbc &= ~(0x1 << i);
        }
...
static void dctrl_tlb_disable(struct dpu_ctrl *dctrl) {
    ...
    reg_write(dctrl, DC_TLB_TLBC, DC_CH0_INVLD | DC_CH1_INVLD | DC_CH2_INVLD | DC_CH3_INVLD);
...
static int wait_dc_state(struct dpu_ctrl *dctrl, uint32_t state, uint32_t flag) {
    ...
    ret = readl_poll_timeout_atomic(dctrl->base + DC_ST, val,
                                    ((!(val & state)) == flag),
                                    10, 200000);
    if (ret) {
    ...
    for (j = 0; j < DPU_SUPPORT_MAX_LAYERS; j++) {
        ...
        */
        ...
        }
        {
            ...
            start_addr += lay_cfg[j].src_crop_y * _stride_in_bytes(lay_cfg[j].stride, lay_cfg[j].format) + lay_cfg[j].src_crop_x * _format_depth_in_bytes(lay_cfg[j].format);

            start_uvaddr += lay_cfg[j].src_crop_y / 2 * _stride_in_bytes(lay_cfg[j].uv_stride, lay_cfg[j].format) + lay_cfg[j].src_crop_x * _format_depth_in_bytes(lay_cfg[j].format);
...
static int dpu_ctrl_alloc_comp_desc(struct dpu_ctrl *dctrl) {
    ...
    for (i = 0; i < DPU_COMP_MAX_FRAMEDESC; i++) {
        for (j = 0; j < DPU_SUPPORT_MAX_LAYERS; j++) {
            dctrl->layerdesc[i][j] = (struct ingenicfb_layerdesc *)(addr + j * size + i * size * DPU_SUPPORT_MAX_LAYERS);
...
int dpu_ctrl_resume(struct dpu_ctrl *dctrl) {
    ...
    if ((dctrl->panel->lcd_type == LCD_TYPE_MIPI_SLCD || dctrl->panel->lcd_type == LCD_TYPE_MIPI_TFT) && dctrl->panel->cur_active_videomode < dctrl->panel->num_modes) {
